...
class PCTransformer(nn.Module):
    ...
    def forward(self, xyz):#[16,2048,3]
        #print("encoder_type", self.encoder_type)
        bs = xyz.size(0)
        coor, f = self.grouper(xyz, self.center_num) # b n c [16,256,3] [16,256,128]
        pe =  self.pos_embed(coor)#[16,256,384]
        x = self.input_proj(f)

        x = self.encoder(x + pe, coor) # b n c [16,256,384]
        global_feature = self.increase_dim(x) # B 1024 N 
        global_feature = torch.max(global_feature, dim=1)[0] # B 1024

        coarse = self.coarse_pred(global_feature).reshape(bs, -1, 3)#[16,768,3]

        coarse_inp = misc.fps(xyz, self.num_query//2) # B 128 3 [16,256,3]
        coarse = torch.cat([coarse, coarse_inp], dim=1) # B 224+128 3? [16,768,3]

        mem = self.mem_link(x)#[16,256,384]

        # query selection
        query_ranking = self.query_ranking(coarse) # b n 1 [b,768,1]
        idx = torch.argsort(query_ranking, dim=1, descending=True) # b n 1
        coarse = torch.gather(coarse, 1, idx[:,:self.num_query].expand(-1, -1, coarse.size(-1)))#[16,512,3]

        if self.training:
            # add denoise task
            # first pick some point : 64?
            picked_points = misc.fps(xyz, 64)#[16,64,3]
            picked_points = misc.jitter_points(picked_points)#[16,64,3]
            coarse = torch.cat([coarse, picked_points], dim=1) # B 256+64 3?#[16,576,3]
            denoise_length = 64     

            # produce query
            q = self.mlp_query(#[16,576,384]
            torch.cat([
            ...
            return q, coarse, denoise_length#[16,576,384] [16,576,3]
...
class AdaPoinTr(nn.Module):
    ...
    def get_loss(self, ret, gt, epoch=1):
        ...
        #print(self.factor)
        idx = knn_point(self.factor, gt, denoised_coarse) # B n k [16,64,32]
        denoised_target = index_points(gt, idx) # B n k 3 [16,64,32,3]
        denoised_target = denoised_target.reshape(gt.size(0), -1, 3)#[16,2048,3]
    ...
    def forward(self, xyz):
        ...
        global_feature = self.increase_dim(q.transpose(1,2)).transpose(1,2) # B M 1024 [b,476,16384]
        global_feature = torch.max(global_feature, dim=1)[0] # B 1024

        rebuild_feature = torch.cat([#[16,576,1411]
        ...
        if self.decoder_type == 'fold':
            ...
            rebuild_feature = self.reduce_map(rebuild_feature) # B M C [16,576,384]
            relative_xyz = self.decode_head(rebuild_feature)   # B M S 3 [16,576,32,3]
            rebuild_points = (relative_xyz + coarse_point_cloud.unsqueeze(-2))  # B M S 3 [16,576,32,3]

        if self.training:
            # split the reconstruction and denoise task
            pred_fine = rebuild_points[:, :-denoise_length].reshape(B, -1, 3).contiguous() #[16,16384,,3]
            pred_coarse = coarse_point_cloud[:, :-denoise_length].contiguous()#[16,512,3]

            denoised_fine = rebuild_points[:, -denoise_length:].reshape(B, -1, 3).contiguous()#[16,2048,3]
            denoised_coarse = coarse_point_cloud[:, -denoise_length:].contiguous()#[16,64,3]

            assert pred_fine.size(1) == self.num_query * self.factor
            assert pred_coarse.size(1) == self.num_query

            ret = (pred_coarse, denoised_coarse, denoised_fine, pred_fine)#[64, 256, 3] [64, 64, 3] [64, 512, 3] [64, 2048, 3]
            #print(pred_coarse.shape,denoised_coarse.shape, denoised_fine.shape, pred_fine.shape)
            return ret
