...
class PCN(nn.Module):
    def __init__(self, config):
        ...
        self.final_conv = nn.Sequential(
            nn.Conv1d(self.encoder_channel+3+2,512,1), #1024+3+2
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Conv1d(512,512,1),
            nn.BatchNorm1d(512),
    ...
    def forward(self, xyz):#[24, 2048, 3]
        ...
        # decoder
        coarse = self.mlp(feature_global).reshape(-1,self.number_coarse,3) # B M 3 [32, 128, 3]
        point_feat = coarse.unsqueeze(2).expand(-1,-1,self.grid_size**2,-1) # B M S 3 [32, 128, 16, 3]
        point_feat = point_feat.reshape(-1,self.number_fine,3).transpose(2,1) # B 3 N [32, 3, 2048]

        seed = self.folding_seed.unsqueeze(2).expand(bs,-1,self.number_coarse, -1) # B 2 M S
        seed = seed.reshape(bs,-1,self.number_fine)  # B 2 N [32, 2, 2048]
