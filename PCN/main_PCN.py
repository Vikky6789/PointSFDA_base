    ...
    for epoch_idx in range(init_epoch + 1, cfg.train.epochs + 1):
        ...
        with tqdm(train_data_loader) as t:
            for batch_idx,data in enumerate(t):
                ...
                ret = model(partial)#[16, 256, 3] [16, 2048, 3]

                loss_coarse, loss_fine = model.module.get_loss(ret, gt)
                ...
                train_writer.add_scalar('Loss/Batch/cd_fine', cd_fine, n_itr)

                batch_time.update(time() - batch_end_time)
                batch_end_time = time()
                t.set_description('[Epoch %d/%d][Batch %d/%d]' % (epoch_idx, cfg.train.epochs, batch_idx + 1, n_batches))

                t.set_postfix(loss='%s' % ['%.4f' % l for l in [cd_coarse,cd_fine]])
        ...
        epoch_end_time = time()
        train_writer.add_scalar('Loss/Epoch/avg_coarse', avg_coarse, epoch_idx)
        train_writer.add_scalar('Loss/Epoch/avg_fine', avg_fine, epoch_idx)

        logging.info(
            '[Epoch %d/%d] EpochTime = %.3f (s) Losses = %s' %
            (epoch_idx, cfg.train.epochs, epoch_end_time - epoch_start_time, ['%.4f' % l for l in [avg_coarse,avg_fine]]))
