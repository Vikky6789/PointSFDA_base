...
#         norm_x = self.norm1(x)
#         norm_y = self.norm2(y)
#         x = x + self.drop_path(self.attn(norm_x+x_pos, norm_x+x_pos,norm_y+y_pos))
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, x, x_pos, y, y_pos,z):
        norm_x = self.norm1(x)
        norm_y = self.norm2(y)
        x = x + self.drop_path(self.attn(norm_x+x_pos, norm_y+y_pos,z))
        x = x + self.drop_path(self.mlp(self.norm3(x)))
        return x
...
#         norm_x = self.norm1(x)
#         norm_y = self.norm2(y)
#         x = x + self.drop_path(self.attn(norm_x+x_pos, norm_y+y_pos,norm_y+y_pos))
#         x = x + self.drop_path(self.mlp(self.norm3(x)))
#         return x
