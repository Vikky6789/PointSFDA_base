...
#from models.Point_MAE_withoutmask import Point_MAE
from utils.loss_utils import get_loss,get_real_loss,get_distill_loss,get_cd,get_ucd
from train import chamfer_distance_native, unidirectional_cd_native
                ...
                if getattr(cfg, 'use_pointmac', False):
                    ...
                    with torch.enable_grad():
                        for _ in range(3):
                            ...
                            mae_rec = aux_outputs['mae_rec']
                            denoise_pred, denoise_target = aux_outputs['denoise_pred'], aux_outputs['denoise_target']
                            
                            # 🔥 Using NaN Shield Native Math for TTA Stability
                            loss_aux1 = chamfer_distance_native(mae_rec, partial, sqrt=False) * 1e2
